...
//TODO goalenter
	...
	void activate_action(account_name from, uint64_t goal_id, eosio::asset quantity){
		...
		auto host = goal->host;		
		rate_index rates(_protocol, host);
		spiral_index spiral(_protocol, host);
	...
	void next(){
		...
		if (ch_goal != chain.end()){
			if (ch_goal -> in_protocol == true){
				//CHANGE PAYER TO _PROTOCOL
				chain.emplace(_protocol, [&](auto &c){
					c.id = chain.available_primary_key();
				...
				//PAYER to _protocol
				chain.modify(ch_goal_next, _protocol, [&](auto &c){
					c.in_protocol = true;
				});
				...
				print("i want send");
		         action(
	                ...
	                std::make_tuple( _self, _protocol, goal->nominal, std::string("null")) 
		        ).send();
    ...
    void apply( uint64_t /*receiver*/, uint64_t code, uint64_t action ) {
      ...
      if (code == _self) {
         ...
         if (action == N(setgoal)){
            set_goal_action(eosio::unpack_action_data<setgoal>());
         }
         if (action == N(editgoal)){
            edit_goal_action(eosio::unpack_action_data<editgoal>());
         }
         if (action == N(delgoal)){
            del_goal_action(eosio::unpack_action_data<delgoal>());
         }
         if (action == N(report)){
            report_action(eosio::unpack_action_data<report>());
         }
         if (action == N(next)){
            next();
         }
         // if (action == N(withdraw)){
         //    withdraw(eosio::unpack_action_data<withdraw>());
         // }
     ...
     if (action == N(transfer))
     {
        if (code == N(eosio.token)){
            ...
            if (op.from != _self){
            	//TODO if op.from == _protocol -> adjust_balance()
                eosio_assert(op.memo != "", "Rejected. Require goal_id in memo field");
                if (op.memo != "null"){
                    goal_id = atoi(op.memo.c_str());                    
                    activate_action(op.from, goal_id, op.quantity);
                }
            }
        } else {
